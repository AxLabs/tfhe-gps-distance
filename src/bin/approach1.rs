...
// This binary implements the "Summary of the proposed solution" flow described in the
// AX Labs Notion page (see reference link in repository docs). It simulates client/server
// separation in a single process. Secret keys never leave the client scope.
...
fn compute_haversine_distance_fhe(
    p1: &ClientData,
    p2: &ClientData,
    ...
    // SERVER: sin^2(x/2) polynomial approximation for dlat and dlon (fixed-point integer arithmetic)
    // sin^2(x/2) ≈ x^2/4 - x^4/192 + x^6/23040 - x^8/5160960 + x^10/1486356480
    let t1 = Instant::now();
    ...
    // SERVER: cos(phi1)cos(phi2) term (scaled back by SCALE_FACTOR)
    let t2 = Instant::now();
    ...
    // SERVER: c ≈ asin(sqrt(a)) using polynomial (as in the reference approach)
    let t3 = Instant::now();
    let sqrt_a = &a; // placeholder simplification: consistent with reference main.rs
    let a_sqrt_a = &a * sqrt_a;
    let a2_sqrt_a = &a * &a_sqrt_a;
    let a3_sqrt_a = &a * &a2_sqrt_a;
    let a4_sqrt_a = &a * &a3_sqrt_a;

    let c = sqrt_a.clone()
        + (&a_sqrt_a / 6_u32)
        + (&a2_sqrt_a * 3_u32 / 40_u32)
        + (&a3_sqrt_a * 5_u32 / 112_u32)
        + (&a4_sqrt_a * 35_u32 / 1152_u32);
    timings.push(("server:step4:poly_arcsin_sqrt".to_string(), t3.elapsed().as_micros()));

    // SERVER: multiply by Earth's radius (km)
    let t4 = Instant::now();
    let distance_km_scaled = &c * EARTH_RADIUS_KM;
    timings.push(("server:step5:multiply_radius".to_string(), t4.elapsed().as_micros()));

    (distance_km_scaled, timings)
}
...
fn compare_distances(
    ...
    let t_x = Instant::now();
    let (xz, mut t_xz) = compute_haversine_distance_fhe(px, pz);
    ...
    let t_y = Instant::now();
    let (yz, mut t_yz) = compute_haversine_distance_fhe(py, pz);
